# Requirements Archive: v1.0 FRD Generator MVP
...
Deferred to future release. Tracked but not in current roadmap. Sourced from v1.0 milestone audit tech debt.

### Production Hardening

- [ ] **PROD-01**: Rate limiting uses persistent store (Redis or equivalent) instead of in-memory sliding window
- [ ] **PROD-02**: Cron cleanup endpoint uses Cloud Scheduler with IAM service account auth instead of Bearer token
- [ ] **PROD-03**: Client-side fetch calls include retry logic for transient 5xx errors

### Code Cleanup

- [ ] **CLEAN-01**: Remove stale TODO comment in `src/lib/db/projects.ts:7` (auth enforcement is complete)
